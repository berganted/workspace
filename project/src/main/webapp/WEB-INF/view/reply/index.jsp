      ...
      <jsp:include page="../include/header.jsp"/>
        <div class="sub">
            <div class="size">
                ...
                <div class="bbs">
                    <table class="list">
                    <span><strong>총 ${replyVo.totCount }개</strong>  |  ${replyVo.reqPage }/${replyVo.totPage }</span>
                        ...
                        <tbody>
							...
							<tr>
								...
								<td><a href="detail.do?no=${list.no }&reqPage=${replyVo.reqPage }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}">
								<c:forEach begin="1" end="${list.nested }">&nbsp;&nbsp;&nbsp; </c:forEach>
								<c:if test="${list.nested > 0}"><img src="/project/img/admin/answer_icon.gif"></c:if>
								${list.title }</a><span style="font-size: 10px">[${list.rcnt }]</span></td>
								<td>${list.name }</td>
								<td>${list.date2 }</td>
                    ...
                    <div class="pagenate clear">
                        <ul class='paging'>
                        	<c:if test="${replyVo.strPage > replyVo.pageRange}"><li><a href="index.do?reqPage=${replyVo.strPage-1 }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}"><</a></li></c:if>
                       	<c:forEach var="rp" begin="${replyVo.strPage }" end="${replyVo.endPage }">
                            <li><a href='index.do?reqPage=${rp }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}' <c:if test="${rp==replyVo.reqPage }">class='current'</c:if>>${rp}</a></li>
                          	...
                          	<c:if test="${replyVo.totPage > replyVo.endPage}"><li><a href="index.do?reqPage=${replyVo.endPage+1 }&stype=${param.stype}&sval=${param.sval}&orderby=${param.orderby}&direct=${param.direct}">></a></li></c:if>      
                        </ul> 
                    </div>
